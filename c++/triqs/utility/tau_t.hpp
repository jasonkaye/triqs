...
namespace triqs::utility {
   ...
   *
   * Represents a point $\tau \in [0, \beta]$ on a very fine integer grid.
  ...
  class tau_t {
    ...
    public:
    /// Maximum grid index, representing $\tau = \beta$.
    ...
    /// Smallest representable positive time, $\epsilon = \beta / N_\mathrm{max}$.
    ...
    /// Cast to double: $\tau = \beta \cdot n / N_\mathrm{max}$.
    ...
    friend tau_t operator-(tau_t a, tau_t b) { return tau_t{a.n - b.n}; }
    friend tau_t operator-(tau_t a) { return tau_t{n_max - a.n}; }
    ...
    /// Create a tau_t from a double value in $[0, \beta]$.
