...
#define ATOM_DIAG_METHOD(RET, F) template <bool Complex> auto atom_diag<Complex>::F->RET
    ...
    template <bool Complex> atom_diag<Complex> atom_diag<Complex>::truncate(double energy_cutoff, int max_states) const {
      ...
      for (int sp = 0; sp < result.n_subspaces(); ++sp)
        for (int i = 0; i < result.get_subspace_dim(sp); ++i)
          all_states.push_back({result.eigensystems[sp].eigenvalues[i], sp, i});
      ...
      for (int op = 0; op < n_ops; ++op) {
        for (auto const &[old_sp, new_sp] : sp_remap) {
          if (long t = result.creation_connection(op, old_sp); t != -1 && sp_remap.contains(t))
            new_creation_connection(op, new_sp) = sp_remap[t];
          if (long t = result.annihilation_connection(op, old_sp); t != -1 && sp_remap.contains(t))
            new_annihilation_connection(op, new_sp) = sp_remap[t];
        }
      }
