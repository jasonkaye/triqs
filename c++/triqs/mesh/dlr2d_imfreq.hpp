...
namespace triqs::mesh {
  ...
  class dlr2d_imfreq {
    ...
    [[nodiscard]] channel_enum channel() const noexcept { return channel_; }

    /// Is the imaginary frequency grid compressed?
    [[nodiscard]] bool compressgrid() const noexcept { return compressgrid_; }

    /// Is the real frequency basis compressed?
    [[nodiscard]] bool compressbasis() const noexcept { return compressbasis_; }

    /// Get the maximum absolute Matsubara index across all mesh points.
    [[nodiscard]] long max_n() const noexcept { return max_n_; }

    /// Get the 1D DLR rank (number of 1D DLR frequencies).
    [[nodiscard]] long rank() const { return (dlr2d_ ? dlr2d_->dlr_rf.size() : 0); }

    /// Get the `nda::vector` of 1D DLR frequencies \f$ \omega_l \f$.
    [[nodiscard]] auto const &dlr_rf() const { return dlr2d_->dlr_rf; }

    /// Get the 2D DLR imaginary frequency grid indices.
    [[nodiscard]] auto const &dlr2d_if() const { return dlr2d_->dlr2d_if; }

    /// Get the 2D DLR real frequency grid indices.
    [[nodiscard]] auto const &dlr2d_rf() const { return dlr2d_->dlr2d_rf; }

    /// Get the coefficients-to-values transformation matrix.
    [[nodiscard]] auto const &cf2if() const { return dlr2d_->cf2if; }
